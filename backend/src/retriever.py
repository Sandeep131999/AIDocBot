...
class Retriever:
    ...
    def __init__(self, 
        ...
        # FIX: Check for existing persisted data instead of assuming empty
        vs_stats = self.vector_store.get_stats()
        self.indexed = vs_stats['total_chunks'] > 0

        if self.indexed:
            print(f"   📚 Found {vs_stats['total_chunks']} existing chunks from {vs_stats['unique_sources']} source(s)")
            print(f"   Sources: {vs_stats['sources']}")
        else:
            print(f"   ⚠️  Vector store is empty — documents need to be indexed")

        # Keyword Search
        self.keyword_searcher = KeywordSearcher()

        # FIX: Rebuild keyword index from persisted vector store if data exists
        if self.indexed:
        ...
        print(f"   Indexed: {'YES (using persisted data)' if self.indexed else 'NO (empty)'}")

    # =====================================================================
    # FIX: Rebuild keyword index from persisted vector store
    # =====================================================================

    def _rebuild_keyword_index(self):
        """Rebuild in-memory keyword index from persisted vector store documents."""
        try:
            print(f"\n[REBUILD KEYWORD INDEX] Restoring from vector store...")
            all_data = self.vector_store.collection.get(include=['documents'])
            documents = all_data.get('documents', [])
            if documents:
                self.keyword_searcher.index_documents(documents)
                print(f"   ✅ Rebuilt keyword index with {len(documents)} documents")
            else:
                print(f"   ⚠️  No documents found to rebuild keyword index")
        except Exception as e:
            print(f"   ⚠️  Could not rebuild keyword index: {e}")
    ...
    def retrieve(self, query: str, top_k: int = None, 
        ...
        if not self.indexed:
            raise ValueError(
                "No documents indexed. Call load_and_index_documents() first, "
                "or check that your VECTOR_DB_PATH points to the correct persistent directory."
            )
        ...
        # FIX: Use relative thresholding for large collections instead of absolute
        filtered = self._apply_relevance_filter(final_results)
    ...
    def _apply_relevance_filter(self, results: List[Dict]) -> List[Dict]:
        """
        Apply relevance filtering with adaptive threshold.
        For large collections, uses relative thresholding to avoid
        returning zero results when absolute scores are low.
        """
        if not results:
            return []

        # Absolute threshold from .env
        abs_threshold = self.min_relevance_score

        # If we have many results and top score is below threshold,
        # use relative thresholding: keep results within a margin of top score
        top_score = results[0]['combined_score'] if results else 0

        if top_score < abs_threshold and len(results) > 3:
            # Relative threshold: keep results within 70% of top score
            relative_threshold = top_score * 0.7
            effective_threshold = min(abs_threshold, max(relative_threshold, 0.15))
            print(f"   📐 Adaptive threshold: {effective_threshold:.3f} (top={top_score:.3f}, abs={abs_threshold})")
        else:
            effective_threshold = abs_threshold

        return [r for r in results if r['combined_score'] >= effective_threshold]
